...
/*  const opcionExportarAtaques = $("<div>Exportar Ataques</div>").appendTo(menuDesplegable);
  $(opcionExportarAtaques).on("click", exportAtaques);
  // Función para mostrar/ocultar el menú desplegable al hacer clic en el botón Extras
  $(btnExtras).on("click", function() {
    $(menuDesplegable).toggle();
  });*/
...
/*  function exportAtaques() {
    const ataques = localStorage.getItem('Atacar');
    if (ataques) {
      var fechaActual = new Date();
      var dia = fechaActual.getDate();
      var mes = fechaActual.getMonth() + 1;
      var año = fechaActual.getFullYear();
      var fechaFormateada = dia + "-" + mes + "-" + año;
      const ataques2 = localStorage.getItem('Imperio');
      var dataJSON = ataques2;
      var objeto = JSON.parse(dataJSON);
      var nombre = objeto.nombre;
      var clan = objeto.clan;
      var nomArchivo = nombre + "_" + clan + "_" + fechaFormateada + ".json";
      descargarDatos(nomArchivo, ataques);
    } else {
      alert('No hay ataques guardadas para exportar.');
    }
  }*/
