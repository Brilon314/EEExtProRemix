...
if (ib <= 15 && LOCAL.getValor() > 500) {
  ...
  $("#datos > tbody > tr:nth-child(3)").append(`<td><b>IB+A:</b> ${aumentoIBAtaque}%</td>`);
  pacifico = true;
} /* if (ib > 15) {
  // $("#contenido > table.lazo.m-reducido > tbody > tr:nth-child(3) > td#contenido > table.lazo.m-reducido > tbody > tr:nth-child(4) > td").append(`<td><b>IB+A:</b> ${aumentoIBAtaque}%</td>`);
  // $("#contenido > table.lazo.m-reducido > tbody > tr:nth-child(4)").append(`<td><b>IB+A:</b> ${aumentoIBAtaque}%</td>`);
  var nuevaFila = `
      <tr>
        <td align="right"><strong>IB+A: ${aumentoIBAtaque}%</strong></td>
      </tr>
    `;

    // Agrega la nueva fila al final del tbody de la tabla con id 'datos'
    $("#datos > tbody > tr:nth-child(3)").append(nuevaFila);
...
if (ib <= 15 && LOCAL.getValor() > 500) {
  var iconoP = `<span id="icono_pacifico"> <img src="//images.empire-strike.com/archivos/icon_paz.gif" width="15" height="15" align="absmiddle" hspace="2" title="Eres un imperio Pacífico"></span>`;
  $("#datos > tbody > tr:nth-child(3)").append(`<td><b>IB+P:</b> ${ibAlPaso}%${iconoP}</td>`);
  pacifico = true;
} else {
  $("#datos > tbody > tr:nth-child(3)").append(`<td><b>IB al paso:</b> ${ibAlPaso}%,${count} paso(s) p pacífico.</td><td> ${aumentoIBAtaque}%</strong></td>`);
