...
function ciudad_process() {
  ...
  setTimeout(() => {
    ...
    for (var i = 0; i < edificios.length; i++) {
      ...
      for (var j = 1; j <= 10; j++) {
        costo.push({
          oro: ciudad_calcular(oroInicial, j),
          material: ciudad_calcular(materialInicial, j),
          recurso: nombreRecurso,
          rentabilidad: renta_edif_base(j, edificios[i]),
        });
      }
...
function ciudad_calcular(inicio, estrella) {
  var result = 0;
  for (var i = 0; i <= estrella; i++) result += inicio * i;
  return result;
}
...
function costoEdificio(nroEstrella, nombre) {
  var gastoTurnos = 2;
  if (LOCAL.getImperio().raza == "Enanos") gastoTurnos = 1;
...
function valorCiudad() {
  ...
  for (var i = 0; i < costosTotales.length; i++) {
    if (edificiosConstruidos[i] == -1) continue;
    var multiplicadorR = 1;
    if (LOCAL.getImperio().raza == "Orcos" && i < 5) multiplicadorR = 0.5;
